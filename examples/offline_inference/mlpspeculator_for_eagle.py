...
# VLLM_USE_V1=0 nohup python examples/offline_inference/mlpspeculator_for_ngram.py &> mlp.log &
import gc
import time
...
if __name__ == "__main__":
    ...
    llm = LLM(
        model=model_name,
        speculative_model="/data/model/yuhuiliEAGLE-LLaMA3-Instruct-8B",
        # ngram_prompt_lookup_max=4,
        num_speculative_tokens=num_speculative_tokens,
        enforce_eager=True
    )
    ...
    with open(f'baseline_times_{file_name}.json', 'a') as f:
        json.dump(times_list1, f, indent=2)
    
    # Save times_list2 (ngram speculative decoding times)
    with open(f'ngram_times_{file_name}.json', 'a') as f:
        json.dump(times_list2, f, indent=2)
    
    # Save speedup ratios
    with open(f'speedup_ratios_{file_name}.json', 'a') as f:
