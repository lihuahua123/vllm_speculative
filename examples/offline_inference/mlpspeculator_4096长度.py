...
def time_generation(llm: LLM, prompts,
    ...
    # llm.generate(prompts, sampling_params)
    start = time.time()
    # guided_decoding_params = GuidedDecodingParams(json=MathResponse.model_json_schema())
...
if __name__ == "__main__":
    ...
    for index,batch_data in tqdm(enumerate(dataloader)):
        if index  in wrong_indexs:
            continue
        # if index not in wrong_indexs_4096:
        #     sampling_params = SamplingParams(ignore_eos=False, max_tokens=max_tokens, temperature=0.8)
        # else:
        #     sampling_params = SamplingParams(ignore_eos=False, max_tokens=max_tokens, temperature=0.0)
        sampling_params = SamplingParams(ignore_eos=False, max_tokens=max_tokens, temperature=0.8)
        finish_count += 1
        ...
        # max_model_len=40560
        # if index in wrong_indexs:
        #     if change_spec_decode_sampler(llm,"rejection_sampler") is None:
        #         print("change spec decode sampler failed1")
        # el
        if index in indexs:
            if change_spec_decode_sampler(llm,"typical_acceptance_sampler") is None:
                print("change spec decode sampler failed2")
            if update_typical_acceptance_threshold(llm,0.8,0.5) is None:
                print("update failed1")
        else:
            if change_spec_decode_sampler(llm,"typical_acceptance_sampler") is None:
                print("change spec decode sampler failed2")
            if update_typical_acceptance_threshold(llm,0.3,0.09) is None:
                print("update failed2")
        elapse_time1,ans1 = time_generation(llm, prompts, sampling_params)
        
        if abs(float(ans1) - float(right_answer.cpu().item())) < 1e-6:
            right+=1
        else:
            print(f"wrong: {index}")
            print(f"ans1: {ans1}, ans2: {right_answer}")
            wrong_ans.append((index,ans1,right_answer))
