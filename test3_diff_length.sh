...
model_name=/root/autodl-tmp/DeepSeek-R1-Distill-Qwen-7B #/root/autodl-tmp/vicuna-13b-v1.3  #/model/vicuna-7b-v1.3
draft_model_name=/root/autodl-tmp/deep05b #/root/autodl-tmp/vllm-medusa-vicuna-7b-v1.3 #/root/autodl-tmp/vicuna-68m #/root/autodl-tmp/vllm-medusa-vicuna-7b-v1.3
...
for i in 1  # 2 3 4 5
do
    for PROMPT_RATE in 15 20 25 30 35 #5 10 15 20 25 #25 # 24 # 6 8 10 12 14 # 10 15 # 20 25 #0.5 2 5 10
    do
       ...
       # python run_benchmark_tests.py --strategy ilp --sub-strategy threshold --model $model_name --save-trace $SAVE_TRACE --draft-model $draft_model_name       --dataset-name $data_set_name         --dataset-path $data_set_path   --speculative-len 2 --num-prompts $NUM_PROMPTS --request-rate $PROMPT_RATE --start-index $START_INDEX --num-gpu-blocks-override $num_gpu_blocks_override --enable-trace "$ENABLE_TRACE" --burstiness $burstiness --gpu-memory-utilization $gpu_memory_utilization &>> $FILE_NAME
        ...
        # kill $pid
        # # smart_spec
        # python run_benchmark_tests.py --strategy ilp --sub-strategy smart_spec  --model $model_name --draft-model $draft_model_name       --dataset-name $data_set_name         --dataset-path $data_set_path   --speculative-len ${MAX_SPECULATIVE_LEN} --num-prompts $NUM_PROMPTS --request-rate $PROMPT_RATE --start-index $START_INDEX --num-gpu-blocks-override $num_gpu_blocks_override --enable-trace "$ENABLE_TRACE" --burstiness $burstiness --gpu-memory-utilization $gpu_memory_utilization &>> $FILE_NAME
        # pid=$(pgrep -f "adaptive_engine_example")
        # kill $pid
        # deep-1
