...
NUM_PROMPTS=10
PROMPT_RATE=10 
FILE_NAME=dynamic_alpaca_llama3_tiaojiao3.log #  134 是100 sharegpt 136 是 $NUM_PROMPTS sharegpt
SPECULATIVE_LEN=4
START_INDEX=0
ENABLE_TRACE="False"
...
# do
    python run_benchmark_tests.py --strategy ilp --sub-strategy ucb --model $model_name --draft-model $draft_model_name       --dataset-name $data_set_name         --dataset-path $data_set_path   --speculative-len $SPECULATIVE_LEN --num-prompts $NUM_PROMPTS --request-rate $PROMPT_RATE --start-index $START_INDEX --num-gpu-blocks-override $num_gpu_blocks_override --enable-trace "$ENABLE_TRACE" --burstiness $burstiness --gpu-memory-utilization $gpu_memory_utilization &>> $FILE_NAME
