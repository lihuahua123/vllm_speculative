...
NUM_PROMPTS=480
PROMPT_RATE=10   
FILE_NAME=dynamicrequest300q_specbench_withincrease.log #  134 是100 sharegpt 136 是 $NUM_PROMPTS sharegpt
SPECULATIVE_LEN=5
START_INDEX=0
ENABLE_TRACE="True"
num_gpu_blocks_override=26064 #4800 4090 # deep A6000 26064 llama8b A6000 xxx  通过下面命令行得到
# python -m vllm.entrypoints.openai.api_server  --model /data/model/Llama-3.1-8B --gpu-memory-utilization 0.85 --speculative-model [ngram] --ngram_prompt_lookup_max 4 --num-speculative-tokens 4 --enforce-eager  --no-enable-prefix-caching --max-model-len 3000
rm -rf $FILE_NAME
...
# for PROMPT_RATE in  1 5 10 #15 20 25 #0.5 2 5 10
# do
...
#python run_benchmark_tests.py --strategy ilp --sub-strategy threshold --model $model_name --draft-model $draft_model_name       --dataset-name $data_set_name         --dataset-path $data_set_path   --speculative-len 3 --num-prompts $NUM_PROMPTS --request-rate $PROMPT_RATE --start-index $START_INDEX --num-gpu-blocks-override $num_gpu_blocks_override --enable-trace "$ENABLE_TRACE" &>> $FILE_NAME
...
# model_name=/root/autodl-tmp/deepseek 
# #model_name=/data/model/deepseek-aiDeepSeek-R1-Distill-Qwen-7B
# draft_model_name=/root/autodl-tmp/DeepSeek-R1-DRAFT-Qwen2.5-0.5B
# #/data/model/DeepSeek-R1-DRAFT-Qwen2.5-0.5B
