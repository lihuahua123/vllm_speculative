...
RESULT_DIR="${RESULT_DIR:-$ROOT_DIR/benchmark_results/long_context_r3_1_2gpu_tp2_48g}"
RUN_LOG="${RUN_LOG:-$RESULT_DIR/long_context_r3_1_2gpu_tp2_48g.log}"
SUMMARY_CSV="${SUMMARY_CSV:-$RESULT_DIR/long_context_r3_1_2gpu_tp2_48g_summary.csv}"
...
SPECULATIVE_DRAFT_TP_SIZE="${SPECULATIVE_DRAFT_TP_SIZE:-2}"
...
# Default to 64K on 2x48GB. 128K can still be enabled explicitly via CLI args.
DEFAULT_CONTEXT_BUCKETS=(64K)
if [ "$#" -gt 0 ]; then
    CONTEXT_BUCKETS=("$@")
else
...
set_profile() {
    ...
    case "$bucket" in
        ...
        64k|64K)
            ...
            NUM_GPU_BLOCKS_OVERRIDE="${NUM_GPU_BLOCKS_OVERRIDE_64K:-4096}"
        ...
        128k|128K)
            ...
            # Leave extra headroom for chat template tokens and KV block capacity.
            RANDOM_INPUT_LEN="${RANDOM_INPUT_LEN_128K:-122880}"
            NUM_PROMPTS=8
            REQUEST_RATES=(0.125 0.25 0.5)
            NUM_GPU_BLOCKS_OVERRIDE="${NUM_GPU_BLOCKS_OVERRIDE_128K:-3584}"
